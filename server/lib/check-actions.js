...
module.exports = async () => {
  ...
  try {
    ...
    if (e.response.status === 401) {
      console.log(
        "Fail with 401 - probably you don't set the GITHUB_ACCESS_TOKEN correctly"
      );
      return;
    }
  }
  ...
  await Promise.all(
    data.workflow_runs
      .filter((run) => !checkedIds[run.id])
      .map(async (run) => {
        let data;
        try {
          const response = await axios.get(
            `https://api.github.com/repos/${process.env.GITHUB_REPO}/actions/runs/${run.id}/jobs`,
            {
              headers: {
                Authorization: `token ${process.env.GITHUB_ACCESS_TOKEN}`,
              },
        ...
        const submissionId = data.jobs[0].name.replace(
          `aa${process.env.ENV_NAME}`,
          ""
        );
        let submission;
        try {
