...
describe("insights tests", () => {
    ...
    expect(body[0].CountSuccessByUser).toBe(2)
    })
    ...
    expect(body[0].state).toBe("SUCCESS")
    expect(body[0].Challenge.id).toBe(2)
    expect(body[0].Challenge.type).toBe(challenges[1].type)
    expect(body[1].id).toBe(1)
    expect(body[1].userId).toBe(1)
    expect(body[1].state).toBe(submissions[0].state)
    expect(body[1].Challenge.id).toBe(1)
    expect(body[1].Challenge.type).toBe(challenges[0].type)
    })    
    ...
    expect(body.length).toBe(2)
    expect(body[1].User.id).toBe(3)
    expect(body[1].User.userName).toBe(users[2].userName)
    expect(body[0][0].id).toBe(2)
    expect(body[0][0].type).toBe(challenges[1].type)
    })
