...
describe('Testing admin insights routes', () => {
  ...
  test('Admin can get the most success submitted challenges of all users', async (done) => {
    ...
    const filteredChallenges = countSuccessSubmissionsPerChallenge(allUsersSubmissionsOrdered, challengesMock).slice(0, 5);
  ...
  test('Admin can get the challenges submissions per challenges of all users', async (done) => {
    ...
    const challengesWithCount = combineSubmissionToChallenge(challengesMock, submissionsMock)
    ...
    const challengesWithCountOnlyLast = combineSubmissionToChallenge(challengesMock, submissionsMock, true)
  ...
  test('Admin can get the challenges submissions per users of all users', async (done) => {
    ...
    const usersFromTeamWithSubmissions = combineSubmissionToUserWithChallenge(usersMock, submissionsMock, challengesMock)
    ...
    const allUsersTeamWithSubmissionsOnlyLast = combineSubmissionToUserWithChallenge(usersMock, submissionsMock, challengesMock, true)

    await timeout(1000);
    console.log('response', teamSubmissionsPerUsersOnlyLast.body);
    console.log('js', allUsersTeamWithSubmissionsOnlyLast);
    await timeout(1000);
    ...
    teamSubmissionsPerUsersOnlyLast.body.forEach((user, index) => {
      expect(allUsersTeamWithSubmissionsOnlyLast[index].Submissions.length).toBe(user.Submissions.length);
    });
