...
describe("Testing teams routes", () => {
    ...
    test("Can teacher add user to team", async (done) => {
        ...
        expect(teamInformation.status).toBe(200);
        expect(teamInformation.body[0].Users.length).toBe(userTeamMock.filter(connection =>
            connection.teamId === teamMock[0].id).length);
        ...
        expect(teamInformationAfterAdded.body[0].Users.length).toBe(userTeamMock.filter(connection =>
            connection.teamId === teamMock[0].id).length + 1);
    ...
    test("Can teacher add user to team", async (done) => {
        ...
        expect(teamInformationAfterRemoved.body[0].Users.length).toBe(userTeamMock.filter(connection =>
            connection.teamId === teamMock[0].id).length - 1);
